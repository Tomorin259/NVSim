...
"""Plot the SERGIO/GNW Ecoli-1200 25-master NVSim bifurcation dataset."""
...
from run_sergio_ecoli1200_25master import ECOLI_1200_DOT, build_sergio_ecoli1200_25master_result
...
OUTPUT_BASE = Path(__file__).with_name("outputs") / "sergio_ecoli1200_25master" / "plots"
...
def main() -> None:
    ...
    noisy = build_sergio_ecoli1200_25master_result(capture_rate=0.6, dropout_rate=0.01, poisson_observed=True)
    lownoise = build_sergio_ecoli1200_25master_result(capture_rate=1.0, dropout_rate=0.0, poisson_observed=False)
    ...
    _, umap_method, _ = compute_umap_embedding(noisy, random_state=2046, layer_preference="observed")
    umap_available = umap_method == "umap"
    if umap_available:
        generated.append(observed_dir / "embedding_umap_observed_by_pseudotime.png")
        plot_embedding_by_pseudotime(noisy, method="umap", layer_preference="observed", random_state=2046, output_path=generated[-1])
        generated.append(observed_dir / "embedding_umap_observed_by_branch.png")
        plot_embedding_by_branch(noisy, method="umap", layer_preference="observed", random_state=2046, output_path=generated[-1])
    for label, gene in selected.items():
    ...
    selected_lines = [
        "Source GRN: SERGIO/GNW_sampled_GRNs/Ecoli_1200_net4.dot",
    ...
    (diagnostics_dir / "README.txt").write_text(
        "SERGIO/GNW Ecoli-1200 25-master quick-look plots. This dataset uses ten branch-driving "
