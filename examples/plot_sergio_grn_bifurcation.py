"""Plot the larger NVSim bifurcation dataset based on a SERGIO/GNW GRN."""
...
from run_sergio_grn_bifurcation import build_sergio_grn_bifurcation_result, load_sergio_dot_grn
...
def main() -> None:
    base_dir = Path(__file__).with_name("outputs") / "sergio_yeast400_3master" / "plots"
    if base_dir.exists():
    ...
    noisy = build_sergio_grn_bifurcation_result(capture_rate=0.6, dropout_rate=0.01, poisson_observed=True)
    lownoise = build_sergio_grn_bifurcation_result(capture_rate=1.0, dropout_rate=0.0, poisson_observed=False)
    ...
    _, umap_method, _ = compute_umap_embedding(noisy, random_state=2026, layer_preference="observed")
    umap_available = umap_method == "umap"
    if umap_available:
        generated.append(observed_dir / "embedding_umap_observed_by_pseudotime.png")
        plot_embedding_by_pseudotime(noisy, method="umap", layer_preference="observed", random_state=2026, output_path=generated[-1])
        generated.append(observed_dir / "embedding_umap_observed_by_branch.png")
        plot_embedding_by_branch(noisy, method="umap", layer_preference="observed", random_state=2026, output_path=generated[-1])
    for label, gene in selected.items():
    ...
    (diagnostics_dir / "README.txt").write_text(
        "Large-GRN quick-look plots from a read-only SERGIO/GNW Yeast-400 DOT network.\n"
        "True plots remain the primary validation view. Observed plots show technical-noise effects.\n"
        "observed_lownoise uses poisson_observed=False for visualization/debugging.\n",
        encoding="utf-8",
    )
