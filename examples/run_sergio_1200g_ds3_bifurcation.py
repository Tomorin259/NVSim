"""Run the SERGIO 1200-gene raw dataset through NVSim's deterministic ODE path.

This is the current recommended SERGIO-style example. It reads SERGIO's
``Interaction`` and ``Regs`` files directly, calibrates half-response values
from SERGIO ``simulated_noNoise`` reference expression, and then runs NVSim's
deterministic bifurcation simulator with state-based source production
profiles.
"""
...
DATASET_DIR = ROOT.parent / "SERGIO" / "data_sets" / "De-noised_1200G_9T_300cPerT_6_DS3"
TARGETS_FILE = DATASET_DIR / "Interaction_cID_6.txt"
REGS_FILE = DATASET_DIR / "Regs_cID_6.txt"
OUTPUT_DIR = Path(__file__).with_name("outputs") / "sergio_1200g_ds3_bifurcation"
OUTPUT_H5AD = OUTPUT_DIR / "sergio_1200g_ds3_bifurcation.h5ad"
SUMMARY_JSON = OUTPUT_DIR / "summary.json"
README_TXT = OUTPUT_DIR / "README.txt"

TRUNK_STATE = "bin_0"
BRANCH_STATES = {"branch_0": "bin_4", "branch_1": "bin_8"}
...
def _write_summary(result: dict, grn_edges: pd.DataFrame, mean_expression: pd.Series) -> dict:
    ...
    summary = {
        "dataset": "SERGIO De-noised_1200G_9T_300cPerT_6_DS3",
    ...
    README_TXT.write_text(
        "\n".join(
            [
                "NVSim SERGIO 1200G deterministic bifurcation output.",
                "",
...
def build_sergio_1200g_ds3_bifurcation_result(
    ...
    result = simulate_bifurcation(
        grn,
        n_trunk_cells=300,
        n_branch_cells={"branch_0": 450, "branch_1": 450},
        trunk_time=2.0,
        branch_time=2.5,
        dt=0.05,
...
def main() -> None:
    ...
    result, grn_edges, mean_expression = build_sergio_1200g_ds3_bifurcation_result()
    try:
