"""scanpy/scVelo-based RNA velocity showcase plots for NVSim outputs.

This module is intentionally separate from ``nvsim.plotting``. The existing
plotting module provides lightweight matplotlib diagnostics, while this module
delegates PCA, neighbors, UMAP, velocity graph, and velocity stream plotting to
scanpy/scVelo.
"""
...
def run_scanpy_embedding(
    ...
    try:
        ...
        raise ImportError("scanpy is required for run_scanpy_embedding") from exc
...
def run_scvelo_velocity_stream(
    adata: Any,
    basis: str = "umap",
    ...
    ``prepare_velocity_adata``. This function does not estimate velocity with
    ...
    try:
        ...
        raise ImportError("scvelo is required for run_scvelo_velocity_stream") from exc

    if velocity_layer not in adata.layers:
        raise KeyError(f"missing velocity layer {velocity_layer!r}")
    if f"X_{basis}" not in adata.obsm:
        raise KeyError(f"missing embedding adata.obsm['X_{basis}']; run run_scanpy_embedding first")
...
def select_velocity_showcase_genes(adata: Any, representative_genes: Any = "auto") -> list[str]:
...
def _plot_phase_portraits(adata: Any, genes: Iterable[str], output_path: Path, dpi: int) -> None:
    genes = list(genes)
    u = np.asarray(adata.layers.get("true_unspliced", adata.layers["unspliced"]), dtype=float)
    s = np.asarray(adata.layers.get("true_spliced", adata.layers["spliced"]), dtype=float)
    v = np.asarray(adata.layers["velocity"], dtype=float)
    pt = adata.obs["pseudotime"].to_numpy(dtype=float) if "pseudotime" in adata.obs else np.arange(adata.n_obs)
    fig, axes = plt.subplots(1, len(genes), figsize=(4.0 * len(genes), 3.6), squeeze=False)
    for ax, gene in zip(axes[0], genes):
        idx = list(map(str, adata.var_names)).index(str(gene))
        sc = ax.scatter(s[:, idx], u[:, idx], c=pt, s=16, cmap="viridis", alpha=0.8)
        order = np.argsort(pt)
        stride = max(1, len(order) // 28)
        q_idx = order[::stride]
        ax.quiver(
            s[q_idx, idx],
            u[q_idx, idx],
            v[q_idx, idx],
            np.zeros_like(v[q_idx, idx]),
            angles="xy",
            scale_units="xy",
            scale=1.0,
            width=0.003,
        ...
        ax.set_xlabel("true spliced")
        ax.set_ylabel("true unspliced")
    fig.colorbar(sc, ax=axes.ravel().tolist(), label="pseudotime")
    fig.suptitle("Representative phase portraits")
    fig.savefig(output_path, dpi=dpi, bbox_inches="tight")
    plt.close(fig)
...
def plot_velocity_showcase(
    ...
    adata = prepare_velocity_adata(data, expression_layer=expression_layer, velocity_layer="true_velocity", copy=True)
    ...
    try:
        run_scvelo_velocity_stream(adata, basis=basis, velocity_layer="velocity")
    ...
    selected = select_velocity_showcase_genes(adata, representative_genes=representative_genes)
