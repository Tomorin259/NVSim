"""Plot the SERGIO 1200-gene raw-dataset NVSim bifurcation example."""
...
from run_sergio_1200g_ds3_bifurcation import (
    BRANCH_STATES,
    DATASET_DIR,
    OUTPUT_DIR,
    OUTPUT_H5AD,
    ...
    TARGETS_FILE,
    TRUNK_STATE,
    build_sergio_1200g_ds3_bifurcation_result,
...
def _load_or_build_noisy():
    ...
    noisy, _, _ = build_sergio_1200g_ds3_bifurcation_result(
        capture_rate=0.6,
        dropout_rate=0.01,
...
def main() -> None:
    ...
    lownoise, _, _ = build_sergio_1200g_ds3_bifurcation_result(
        capture_rate=1.0,
        dropout_rate=0.0,
    ...
    _, umap_method, _ = compute_umap_embedding(noisy, random_state=2060, layer_preference="observed")
    umap_available = umap_method == "umap"
    if umap_available:
        generated.append(observed_dir / "embedding_umap_observed_by_pseudotime.png")
        plot_embedding_by_pseudotime(
            noisy, method="umap", layer_preference="observed", random_state=2060, output_path=generated[-1]
        )
    ...
    (diagnostics_dir / "README.txt").write_text(
        "SERGIO original 1200-gene targets/regs quick-look plots routed through NVSim deterministic ODE. "
