...
Use `simulate()` for a single continuous trajectory:
...
Use `simulate(..., simulator="bifurcation")` for a trunk plus two child branches. The canonical `state_anchor` interface is:
...
`simulate_linear()` and `simulate_bifurcation()` remain available as backward-compatible wrappers, but new code should prefer `simulate()`.
...
package. New code should call `simulate(..., simulator="bifurcation")`
or `simulate_bifurcation()`.
...
单一连续轨迹优先用 `simulate(..., simulator="linear")`。

trunk 加两条子分支优先用 `simulate(..., simulator="bifurcation")`。
...
`simulate_linear()` 和 `simulate_bifurcation()` 仍保留为兼容包装层，但新代码建议优先写成统一的 `simulate()` 调用。
...
helper；它不会从顶层包重新导出。新的调用方式应优先使用
`simulate(..., simulator="bifurcation")`，也可以继续使用
