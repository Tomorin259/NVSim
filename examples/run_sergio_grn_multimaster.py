...
"""Run NVSim on SERGIO/GNW Yeast-400 with multiple branch-driving masters.

This example keeps the SERGIO source tree read-only and converts the
Yeast-400 DOT network into NVSim's GRN schema. Compared with
run_sergio_grn_bifurcation.py, this version drives branch programs through
ten master regulators instead of three so the simulated manifold has a
...
N_BRANCH_MASTERS = 10
OUTPUT_DIR = Path(__file__).with_name("outputs") / "sergio_yeast400_multimaster"
...
def build_sergio_grn_multimaster_result(
    capture_rate: float = 0.6,
    dropout_rate: float = 0.01,
    poisson_observed: bool = True,
    n_branch_masters: int = N_BRANCH_MASTERS,
):
    """Generate a Yeast-400 bifurcation dataset with multiple dynamic masters."""
    ...
    result = simulate_bifurcation(
        grn,
        n_trunk_cells=220,
        n_branch_cells={"branch_0": 260, "branch_1": 260},
        trunk_time=2.0,
        branch_time=2.5,
        dt=0.05,
...
def main() -> None:
    result = build_sergio_grn_multimaster_result()
    ...
    try:
        ...
        out = OUTPUT_DIR / "sergio_yeast400_multimaster_bifurcation.h5ad"
